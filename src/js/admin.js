...
document.querySelector('#icw-fix-content-btn').addEventListener('click', async (event) => {
	...
	const getProgressBar = (time, times, batchSize, count) => {
		...
		const completed = time !== times ? batchSize * time : count;
		return `<span>[${'#'.repeat(value)}${'-'.repeat(
			progressBarMax - value,
		)}] | ${percentage}% || ${completed}/${count} ${__(
			'posts',
